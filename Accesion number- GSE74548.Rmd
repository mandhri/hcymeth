...
mx <-Mval_flt
MX <-Mval
colnames(MX)<-sapply(strsplit(colnames(MX),"_"),"[[",1)
colnames(mx)<-sapply(strsplit(colnames(mx),"_"),"[[",1)
myscree(Mval,main="incl sex chr")
myscree(mx,main="excl sex chr")
...
mydist <- plotMDS(MX, labels=targets$Sample_Name,col=colours,main="sex chromosomes included")
mydist_flt <- plotMDS(mx, labels=targets$Sample_Name,col=colours,main="sex chromosomes excluded")
```
...
table(age_groups)
colour_palette=brewer.pal(n = length(levels(targets$hcy_groups)), name = "Paired")
colours <- colour_palette[as.integer(factor(targets$hcy_groups))]
plot(1,axes = FALSE,xlab="",ylab="",main="hcy levels")
legend("center",legend=levels(targets$hcy_groups),pch=16,cex=1.2,col=colour_palette)
mydist <- plotMDS(MX, labels=targets$Sample_Name,col=colours,main="sex chromosomes included")
mydist_flt <- plotMDS(mx, labels=targets$Sample_Name,col=colours,main="sex chromosomes excluded")
```
```{r,hist}
...
colour_palette=brewer.pal(n = length(levels(targets$age)), name = "Paired")
colours <- colour_palette[as.integer(factor(targets$age))]
plot(1,axes = FALSE,xlab="",ylab="",main="hcy levels")
legend("center",legend=levels(targets$age),pch=16,cex=1.2,col=colour_palette)
mydist <- plotMDS(MX, labels=targets$Sample_Name,col=colours,main="sex chromosomes included")
mydist_flt <- plotMDS(mx,labels=targets$Sample_Name,col=colours,main="sex chromosomes excluded")
```
...
colour_palette=brewer.pal(n = length(levels(targets$characteristics_ch1.8)), name = "Paired")
